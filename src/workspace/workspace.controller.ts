...
import { ApiBearerAuth, ApiBody, ApiConsumes, ApiOkResponse, ApiTags, getSchemaPath } from '@nestjs/swagger';
import { JwtAuthGuard } from 'src/auth/jwt-auth.guard';
import { PermissionsGuard } from 'src/auth/permissions.guard';
import { Permissions } from '../auth/roles.decorator';
import { WorkspaceService } from './workspace.service';
import { CreateWorkspaceDto, ResponseFullWorkspaceDto, ResponseWorkspaceDto, UpdatePS } from './dto/workspace.dto';
...
export class WorkspaceController {
    ...
    async listMine(@Req() req: { user: { userId: string, rolePermissions: RolePermission[] } }): Promise<ResponseWorkspaceDto[]> {
        if (req.user.rolePermissions.find((rp) => rp.permission.code === PermissionCodes.workspaceManage && rp.allowed)) {
    ...
    @UseGuards(WorkspaceGuard)
    async addUser(@Param('workspaceId') workspaceId: string, @Param('userId') userId: string) {
    ...
    async updatePS(@Param('workspaceId') id: string, @Body() data: UpdatePS, @UploadedFile() file?: any) {
